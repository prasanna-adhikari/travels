...
export default function City({ length }) {
  const [countries, setCountries] = useState();
  const getCountry = async () => {
    try {
      if (length == 6) {
        console.log(length);
        const response = await publicFetch.get(`/api/v1/country?limit=2`);

        setCountries(response.data);
      } else {
        const response = await publicFetch.get(`/api/v1/country`);
                ...
                >
                  <Flex
                    ...
                    backgroundImage={
                      ...
                      `url(${BASE_URL}${country.countryImage.replace(
                        "\\",
                        "/"
                      )})`
                    }
